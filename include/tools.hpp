...
static Eigen::Matrix<double, DIMU, DIMU> I_imu(Eigen::Matrix<double, DIMU, DIMU>::Identity());
static Eigen::Matrix<double, 12, 12> I12(Eigen::Matrix<double, 12, 12>::Identity());
...
struct IMUST {
    double t;
    Eigen::Matrix3d R;  //rot
    Eigen::Vector3d p;  //trans
    Eigen::Vector3d v;   //scale
    Eigen::Vector3d bg;
    Eigen::Vector3d ba;
    Eigen::Vector3d g;
    ...
    IMUST(double _t, const Eigen::Matrix3d &_R, const Eigen::Vector3d &_p, const Eigen::Vector3d &_v,
          ...
          const Eigen::Vector3d &_g = Eigen::Vector3d(0, 0, -G_m_s2)) : t(_t), R(_R), p(_p), v(_v), bg(_bg),
                                                                        ba(_ba), g(_g) {}
...
public:
    ...
    void delete_clus(const PointCluster &cluster) {
        N-=cluster.N;
        P -= cluster.P;
        v -= cluster.v;
    }
    ...
    inline void transform(const PointCluster &sigv, const IMUST &stat) {
        N = sigv.N;
